    ...
    nuxt.hook("modules:done", async () => {
      ...
      if (config.header) {
        ...
        Object.entries(nuxt.options.routeRules).forEach(([route, rules]) => {
          const robotRule = util_js.normaliseRobotsRouteRule(rules);
      ...
      if (config.disallowNonIndexableRoutes) {
        Object.entries(nuxt.options.routeRules || {}).forEach(([route, rules]) => {
          ...
          const robotsRule = util_js.normaliseRobotsRouteRule(rules);
...
`;
    ...
    if ((isNuxtGenerate() || isFirebase && nuxt.options._build) && config.robotsTxt) {
      ...
      nuxt.options.generate.routes = util_js.asArray(nuxt.options.generate.routes || []);
