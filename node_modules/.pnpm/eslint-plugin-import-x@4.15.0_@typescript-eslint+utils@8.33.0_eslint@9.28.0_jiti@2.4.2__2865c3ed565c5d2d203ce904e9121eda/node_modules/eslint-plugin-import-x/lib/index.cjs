...
const stable_hash = __toESM(require("stable-hash"));
...
function fullResolve(modulePath, sourceFile, settings, context) {
	...
	if (prevSettings !== settings) {
		memoizedHash = (0, stable_hash.stableHash)(settings);
...
function makeContextCacheKey(context) {
	...
	let hash = (0, stable_hash.stableHash)(settings) + (0, stable_hash.stableHash)(languageOptions?.parserOptions ?? parserOptions);
	if (languageOptions) hash += String(languageOptions.ecmaVersion) + String(languageOptions.sourceType);
	hash += (0, stable_hash.stableHash)(parserPath ?? languageOptions?.parser?.meta ?? languageOptions?.parser);
	return hash;
}
...
function testConfig(config, filename) {
	...
	return config.some((c) => (0, minimatch.minimatch)(filename, c) || (0, minimatch.minimatch)(filename, node_path.default.resolve(c), { windowsPathsNoEscape: true }));
}
	...
	create(context) {
		...
		return {
			...
			MemberExpression(node) {
				...
				if (isIdentifier && hasCJSExportReference && !isEntryPoint && !options.exceptions?.some((glob) => (0, minimatch.minimatch)(filename, glob)) && !isImportBinding) {
...
var no_namespace_default = createRule({
	...
	create(context) {
		...
		return { ImportNamespaceSpecifier(node) {
			if (ignoreGlobs?.find((glob) => (0, minimatch.minimatch)(node.parent.source.value, glob, { matchBase: true }))) return;
...
function testIsAllow(globs, filename, source) {
	...
	return globs.some((glob) => (0, minimatch.minimatch)(filePath, glob, { nocomment: true }) || (0, minimatch.minimatch)(filePath, node_path.default.resolve(glob), {
		nocomment: true,
