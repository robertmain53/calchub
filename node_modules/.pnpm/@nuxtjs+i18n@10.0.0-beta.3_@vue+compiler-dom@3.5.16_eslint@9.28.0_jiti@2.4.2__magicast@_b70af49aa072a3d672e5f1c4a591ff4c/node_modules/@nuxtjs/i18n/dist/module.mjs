import { directoryToURL, resolveModule, useLogger, resolvePath, addTemplate, tryUseNuxt, addBuildPlugin, useNuxt, addWebpackPlugin, addRspackPlugin, extendViteConfig, addServerTemplate, addServerImports, addServerPlugin, createResolver, addPlugin, addVitePlugin, useNitro, addTypeTemplate, addComponent, addImports, defineNuxtModule } from '@nuxt/kit';
import createDebug from 'debug';
import { readFileSync, existsSync } from 'node:fs';
import { isObject, assign, isString, isArray } from '@intlify/shared';
import { parse as parse$1, compileScript } from '@vue/compiler-sfc';
import { walk } from 'estree-walker';
import { mkdir, readFile, writeFile } from 'node:fs/promises';
import defu$1, { defu } from 'defu';
import { createHash } from 'node:crypto';
import { resolve, dirname, parse, isAbsolute, basename, relative, join as join$1 } from 'pathe';
import { parseSync } from 'oxc-parser';
import { encodePath, parseURL, parseQuery } from 'ufo';
import { getLocalizedRouteName } from '../dist/runtime/kit/routing.js';
...
import { genArrayFromRaw, genObjectFromRaw, genString, genObjectFromValues, genSafeVariableName, genImport, genDynamicImport } from 'knitwork';
...
function setupAlias({ userOptions: options }, nuxt) {
  ...
  for (const [moduleName, moduleFile] of Object.entries(modules)) {
    ...
    debug$8(`${moduleName} alias`, nuxt.options.alias[moduleName]);
  }
}
...
function filterLocales(ctx, nuxt) {
  ...
  ctx.options.locales = ctx.options.locales.filter(
...
const mergeI18nModules = async (ctx, nuxt) => {
  const i18nModules = [];
  await nuxt.callHook(
    "i18n:registerModule",
...
const debug$7 = createDebug(NUXT_I18N_MODULE_ID);
...
function createLocalizeRouteName(opts) {
  return (route, locale, isDefault) => getLocalizedRouteName(route.name, locale, isDefault, opts.routesNameSeparator, opts.defaultLocaleRouteNameSuffix);
}
...
async function setupPages({ localeCodes, options }, nuxt) {
...
function getRouteFromMacro(_ctx, route, localeCodes) {
...
function getRouteOptions(route, localeCodes, ctx, defaultLocale, mode = "config") {
  const resolvedOptions = mode === "config" ? getRouteFromConfig(ctx, route, localeCodes) : getRouteFromMacro(ctx, route, localeCodes);
...
const debug$3 = createDebug("@nuxtjs/i18n:function:injection");
    ...
    transform: {
      ...
      handler(code, id) {
        ...
        if (missingFunctionDeclarators.size > 0) {
          debug$3(`injecting ${Array.from(missingFunctionDeclarators).join(", ")} declaration to ${id}`);
          const assignments = [];
...
function getDefineConfig(options, server = false, nuxt = useNuxt()) {
  const common = {
...
const debug$1 = createDebug("@nuxtjs/i18n:layers");
...
function resolveI18nDir(layer, i18n) {
...
function applyLayerOptions(ctx, nuxt) {
  ctx.options.locales ??= [];
  const configs = [];
  for (const layer of nuxt.options._layers) {
  ...
  debug$1("merged locales", configs);
...
async function resolveLocaleInfo(ctx, nuxt) {
  ...
  debug$7("localeInfo", ctx.localeInfo);
  ctx.vueI18nConfigPaths = await resolveLayerVueI18nConfigInfo(ctx.options);
  debug$7("VueI18nConfigPaths", ctx.vueI18nConfigPaths);
}
...
function generateI18nTypes(nuxt, { userOptions: options, normalizedLocales, runtimeDir, distDir }) {
...
function prepareStrategy({ options, normalizedLocales }, nuxt) {
