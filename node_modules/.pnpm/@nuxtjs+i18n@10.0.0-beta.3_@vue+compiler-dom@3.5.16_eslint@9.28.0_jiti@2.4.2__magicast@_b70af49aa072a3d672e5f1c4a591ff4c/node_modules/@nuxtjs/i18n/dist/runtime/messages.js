...
export async function loadVueI18nOptions(vueI18nConfigs, nuxt = nuxtMock) {
...
export async function loadInitialMessages(messages, localeLoaders, options, nuxt = nuxtMock) {
  ...
  const locales = __LAZY_LOCALES__ ? [...(/* @__PURE__ */ new Set()).add(defaultLocale).add(initialLocale)] : localeCodes;
  await Promise.all(locales.map((locale) => loadAndSetLocaleMessages(locale, localeLoaders, messages, nuxt)));
  return messages;
}
const isModule = (val) => toTypeString(val) === "[object Module]";
async function loadMessage(locale, { key, load }, nuxt = nuxtMock) {
...
export async function loadLocale(locale, localeLoaders, setter, nuxt = nuxtMock) {
...
export async function loadAndSetLocaleMessages(locale, localeLoaders, messages, nuxt = nuxtMock) {
